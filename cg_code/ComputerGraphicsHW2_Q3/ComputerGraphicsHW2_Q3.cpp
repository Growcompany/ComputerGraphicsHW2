...
public:
    ...
    Ray getRay(int ix, int iy, float offsetX, float offsetY) const {
        float u = l + (r - l) * (ix + offsetX) / nx;
        float v = b + (t - b) * (iy + offsetY) / ny;
        vec3 pixelPos(u, v, -d);
...
void render() {
    ...
    for (int iy = 0; iy < Height; ++iy) {
        for (int ix = 0; ix < Width; ++ix) {
            ...
            for (int s = 0; s < AntialiasingPerPixel; ++s) {
                float offsetX = static_cast<float>(rand()) / RAND_MAX; // [0,1)
                float offsetY = static_cast<float>(rand()) / RAND_MAX;

                // Create ray
                Ray ray = camera.getRay(ix, iy, offsetX, offsetY);
            ...
            finalColor /= float(AntialiasingPerPixel); // box filter Æò±Õ
